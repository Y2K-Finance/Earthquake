...
contract VaultV2PausableTest is Helper {
    VaultV2Pausable vault;
    VaultV2Pausable counterpartyVault;
    address controller;
    ...
    function setUp() public {
        ...
        counterpartyVault = new VaultV2Pausable(
            false,
            UNDERLYING,
            "Vault",
            "v",
    ...
    function testVaultCreation() public {
        ...
        vm.expectRevert(VaultV2Pausable.AddressZero.selector);
        new VaultV2Pausable(
            false,
        ...
        // test success case
        VaultV2Pausable vault2 = new VaultV2Pausable(
            false,
            UNDERLYING,
            "Vault",
            "v",
    ...
    function testSetEpochPausable() public {
        // test revert cases

        vm.startPrank(NOTADMIN);
        vm.expectRevert(
            abi.encodeWithSelector(
                VaultV2Pausable.AddressNotFactory.selector,
                NOTADMIN
            )
        );
        ...
        vm.expectRevert(VaultV2Pausable.InvalidEpoch.selector);
    ...
    function testDepositPausable() public {
        // test revert cases
    ...
    function testResolveEpoch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VaultV2Pausable.AddressNotController.selector,
                NOTADMIN
            )
        );
        vault.resolveEpoch(epochId);
        ...
        vm.warp(begin - 1);
        vm.expectRevert(VaultV2Pausable.EpochNotStarted.selector);
        vault.resolveEpoch(epochId);

        vm.warp(begin + 1);
        vault.resolveEpoch(epochId);
        vm.expectRevert(VaultV2Pausable.EpochAlreadyEnded.selector);
        vault.resolveEpoch(epochId);
    ...
    function testClaimTVL() public {
        ...
        vm.expectRevert(VaultV2Pausable.InvalidClaimTVL.selector);
    ...
    function testSendTokens() public {
        ...
        vm.expectRevert(VaultV2Pausable.AmountExceedsTVL.selector);
        vault.sendTokens(epochId, 11 ether, address(counterpartyVault));

        vm.expectRevert(
            abi.encodeWithSelector(
                VaultV2Pausable.DestinationNotAuthorized.selector,
                NOTADMIN
            )
        );
