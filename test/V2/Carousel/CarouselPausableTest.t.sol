...
import "../Helper.sol";
import "../../../src/v2/Carousel/CarouselPausable.sol";
import "../../../src/v2/libraries/CarouselCreatorPausable.sol";
import "../../../src/v2/interfaces/ICarousel.sol";
import "../../../src/v2/VaultV2Pausable.sol";
import "forge-std/Test.sol";

contract CarouselPausableTest is Helper {
    using stdStorage for StdStorage;

    // VaultV2 vaultv2;
    CarouselPausable vault;
    ...
    function testDepositInQueuePausable() public {
        ...
        // revert case
        // can not deposit if market is paused
        vault.pauseMarket();
        ...
        vm.expectRevert(CarouselPausable.MinDeposit.selector);
        ...
        vm.startPrank(USER2);
        address faultyReceiver = address(
            new MintableToken("faulty contract", "faulty")
        );
        IERC20(UNDERLYING).approve(address(vault), 10 ether);
    ...
    function testEnListInRolloverPausable() public {
        // create two epochs
        uint40 _epochBegin = uint40(block.timestamp + 1 days);
        uint40 _epochEnd = uint40(block.timestamp + 2 days);
        uint256 _epochId = 2;
        ...
        // test revert case
        // should not be able to enlist when paused
        vault.pauseMarket();
        vm.expectRevert(VaultV2Pausable.MarketPaused.selector);
        vault.enlistInRollover(_epochId, 900000000000000000, USER);
        vault.pauseMarket();
        ...
        // relayer should not be able to mint rollovers when paused
        vm.startPrank(relayer);
        vm.expectRevert(VaultV2Pausable.MarketPaused.selector);
    ...
    function testDepositIntoQueueMultiplePausable() public {
    ...
    function testRolloverMultiplePausable() public {
        // test multiple rollovers
        // roll over users from testDepositIntoQueueMultiple test
        testDepositIntoQueueMultiplePausable();
        ...
        vm.expectRevert(VaultV2Pausable.EpochNotResolved.selector);
        ...
        uint256 prevEpochShareValue = vault.previewWithdraw(
            prevEpoch,
            vault.balanceOf(USER, prevEpoch)
        );
        ...
        uint256 prevEpochSharesValueAfterRollover = vault.previewWithdraw(
            prevEpoch,
            vault.balanceOf(USER, prevEpoch)
        );
        assertEq(
            ((prevEpochSharesValueAfterRollover >> 1) << 1),
            (((prevEpochShareValue) - (prevEpochUserBalance) - 16) >> 1) << 1
        ); // zero out last bit to avoid rounding errors
        // check balances
        assertEq(
            vault.balanceOf(USER, _epochId),
            prevEpochUserBalance - relayerFee
        );
        assertEq(
            vault.balanceOf(USER2, _epochId),
            prevEpochUserBalance - relayerFee
        );
        assertEq(
            vault.balanceOf(USER3, _epochId),
            prevEpochUserBalance - relayerFee
        );
        assertEq(
        ...
        assertEq(
            vault.balanceOf(USER5, _epochId),
            prevEpochUserBalance - relayerFee
        );
        assertEq(
            vault.balanceOf(USER6, _epochId),
            prevEpochUserBalance - relayerFee
        );
    }
    ...
    function helperDepositInEpochs(
        uint256 _epoch,
        address user,
        bool queue
    ) public {
    ...
    function helperRolloverFromEpoch(
        uint256 _epoch,
        address user,
        uint256 amount
    ) public {
