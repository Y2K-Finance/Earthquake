...
contract CarouselFactoryPausableTest is Helper {
    ...
    function testCarouselEpochDeploymentPausable() public {
        ...
        assertEq(
            MintableToken(emissionsToken).balanceOf(vaults[0]),
            premiumEmissions
        );
        assertEq(
            MintableToken(emissionsToken).balanceOf(vaults[1]),
            collatEmissions
        );
    }
    ...
    function testChangeRelayerFeePausable() public {
        ...
        factory.changeRelayerFee(newFee, marketId);
        assertEq(
            ICarousel(factory.getVaults(marketId)[0]).relayerFee(),
            newFee
        );
        assertEq(
            ICarousel(factory.getVaults(marketId)[1]).relayerFee(),
            newFee
        );
    ...
    function testChangeDepositFeePausable() public {
        ...
        factory.changeDepositFee(newFee, marketId, 0);
        assertEq(
            ICarousel(factory.getVaults(marketId)[0]).depositFee(),
            newFee
        );
        // check if collateral vault fee is not changed
        assertEq(
            ICarousel(factory.getVaults(marketId)[1]).depositFee(),
            depositFee
        );
