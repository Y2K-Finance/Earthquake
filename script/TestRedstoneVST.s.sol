...
contract TestRedstoneVST is Script {
    ...
    function getJsPrice(
        string memory redstoneOracleAddress
    ...
    function run() public {
        ...
        if (!isNumber(newPrice)) {
            ...
            try rpp.getLatestPrice(marketId) returns (int256 vstPrice) {
    ...
    function getConfigAddresses(
        bool isTestEnv
    ) public returns (ConfigAddresses memory) {
        ...
        ConfigAddresses memory rawConstants = abi.decode(
            parseJsonByteCode,
            (ConfigAddresses)
        );
