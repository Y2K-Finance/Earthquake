...
contract ControllerGeneric {
    ...
    address public immutable y2kTreasury;

    constructor(address _factory, address _treasury, address _y2kTreasury) {
        if (_factory == address(0)) revert ZeroAddress();
        if (_treasury == address(0)) revert ZeroAddress();
        vaultFactory = IVaultFactoryV2(_factory);
        treasury = _treasury;
        y2kTreasury = _y2kTreasury;
    }
    ...
    function triggerLiquidation(uint256 _marketId, uint256 _epochId) public {
        ...
        // TODO: Calc fee for Y2K
        premiumVault.sendTokens(_epochId, premiumFee / 2, y2kTreasury);
        premiumVault.sendTokens(_epochId, premiumFee / 2, treasury);
        ...
        // TODO: Calc fee for Y2K
        collateralVault.sendTokens(_epochId, collateralFee / 2, y2kTreasury);
        collateralVault.sendTokens(_epochId, collateralFee / 2, treasury);
    ...
    function triggerEndEpoch(uint256 _marketId, uint256 _epochId) public {
        ...
        // send fees to y2kTreasury - TODO: Calc fee for Y2K
        premiumVault.sendTokens(_epochId, premiumFee / 2, y2kTreasury);
        ...
        emit ProtocolFeeCollected(_epochId, _marketId, premiumFee, 0);
    }
