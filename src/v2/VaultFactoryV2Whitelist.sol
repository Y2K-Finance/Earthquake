...
contract VaultFactoryV2 is Ownable {
    ...
    mapping(address => mapping(uint256 => bool)) public epochManager;
    ...
    function _createNewMarket(
        ...
        // Set the caller as a market manager
        epochManager[msg.sender][marketId] = true;
    ...
    /**
    @notice Owner can whitelist an address to deploy markets
    ...
    /**
    @notice Market deployers can add managers for a market they are manager of
    @param _manager List of the managers
    @param _marketId Target marketId
     */
    function updateEpochManager(
    ...
    /** @notice Modifer to check caller is a manager of the marketId
     */
    modifier onlyEpochManager(uint256 marketId) {
        if (!epochManager[msg.sender][marketId]) revert NotManager();
        _;
    }
    ...
    /** @notice Modifer to check the caller is a whitelisted market deployer
     */
    modifier deployerIsWhitelisted() {
        if (!whitelistedDeployer[msg.sender]) revert NotDeployer();
        _;
    }
